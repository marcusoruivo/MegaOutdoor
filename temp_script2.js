...
function renderHeader(){
    ...
    const btnBisbilhotar = !ehMeu && !p.privado
        ? '<button class="btn btn-amarelo" style="margin-top:10px;padding:8px 14px;font-size:12px" onclick="bisbilhotarAlbum()">👀 BISBILHOTAR</button>'
        : '';
    document.getElementById("perfilHeader").innerHTML =
        '<div class="header-perfil">' +
            ...
            '</div>' +
            (btnBisbilhotar ? '<div style="margin-top:10px;text-align:center">' + btnBisbilhotar + '</div>' : '') +
        '</div>';
}
